...
// When the feature IS present but the destination cannot be verified (nil pipeline/node), Require
// must fail closed -- never silently allow the send. The message is derived from name + version, so
// the version number is not duplicated as literal text.
